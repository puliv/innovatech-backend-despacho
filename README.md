...
Instalación
    ...
    Clonar el repositorio: git clone https://github.com/puliv/innovatech-backend-despacho
    Acceder a la rama de despliegue: git checkout deploy
    Instalar las dependencias de producción: npm install --production
...
Para iniciar el servicio en el entorno de producción, utilice los siguientes comandos:
    ...
    Compilación (si el proyecto usa TypeScript): npm run build
    Inicio del servicio: npm start
    Ejecución en segundo plano (Recomendado): Si utiliza PM2: pm2 start dist/main.js --name backend-despacho
